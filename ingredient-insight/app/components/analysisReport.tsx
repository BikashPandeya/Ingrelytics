...
const SafetyProgressBar: React.FC<{ score10: number }> = ({ score10 }) => {
  const pct = Math.min(100, Math.max(0, (score10 / 10) * 100))
  const barColor = score10 >= 7.5 ? 'bg-green-500' : score10 >= 5 ? 'bg-yellow-500' : 'bg-red-500'
  ...
  return (
    <div className="w-full bg-gray-300 rounded-full h-3 overflow-hidden">
      <div ref={barRef} className={`h-3 rounded-full transition-all duration-500 ${barColor}`} />
    </div>
  )
}
...
const AnalysisReportComponent: React.FC<AnalysisReportProps> = ({ report, onReset }) => {
  const getSafetyColor = (score: number) => {
    if (score >= 7.5) return { bg: 'bg-green-50', border: 'border-green-300', text: 'text-green-700', badge: 'bg-green-200 text-green-800' }
    if (score >= 5) return { bg: 'bg-yellow-50', border: 'border-yellow-300', text: 'text-yellow-700', badge: 'bg-yellow-200 text-yellow-800' }
  ...
  const scoreOutOf10 = Number(report.safetyScore.toFixed(1))
  const safetyColor = getSafetyColor(scoreOutOf10)
  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-6 animate-fadeIn">
      <div className={`p-6 rounded-lg border-2 ${safetyColor.bg} ${safetyColor.border}`}>
        ...
        <SafetyProgressBar score10={scoreOutOf10} />
